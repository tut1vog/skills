...
## Fit check
Offload only tasks that are **self-contained**: inputs come from the network (public datasets, git URLs, package registries) and outputs are files the tool fetches back. The tool does not upload the local workspace. If the task needs local files as input, run it locally or ask the user to publish the inputs.
...
6. **Collect**: `offload fetch <job-id>` downloads `$OUT` to `./offload-results/<job-id>/`. Pass `--to <dir>` to place results elsewhere, or a remote path to grab something outside `$OUT`.
